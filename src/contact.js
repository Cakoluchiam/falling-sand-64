...
export class ContactSolver {
   ...
   * `friction`, `restitution` and `iterations`.
   */
  step(P, field, dt, o) {
    ...
    const xprev = this.xprev, approachVn = this.approachVn, touching = this.touching;
    ...
    // --- Project out of the surface, then apply the friction cone.
    for (let it = 0; it < iterations; it++) {
      for (let k = 0; k < n; k++) {
        const i = live[k];
        if (phase[i] !== PHASE_AWAKE || !touching[i]) continue;
        const p = i * 4;
    ...
    for (let k = 0; k < n; k++) {
      ...
      if (target > vn) {
        const add = target - vn;
        vx[i] += nx * add; vy[i] += ny * add; vz[i] += nz * add;
      }
    }
  }
}
