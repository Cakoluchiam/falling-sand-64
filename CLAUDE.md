...
- **Two pieces of code that describe the same trajectory have to agree exactly.** The nozzle backdates a grain onto the analytic free-fall curve for its sub-frame age, and the integrator has to walk that same curve or the stream tears where they hand off. `x += v_new * dt` is a step behind — it overshoots constant acceleration by `½·a·dt²` every step — so the position update uses the average of the two velocities. This is why `src/ballistic.js` is its own module: a test has to be able to reach it.
