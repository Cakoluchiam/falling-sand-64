...
> 9. *(version 5)* **The flight integrator advances position on the average of its two velocities**, so it walks the curve the backdate assumes instead of overshooting it. Extracted to `src/ballistic.js` so the invariant can be tested. See the Source decisions.
